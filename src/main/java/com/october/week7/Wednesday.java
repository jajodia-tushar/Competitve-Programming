...
public class Wednesday {
   ...
   edges.
    ...
    public static int findTheNumberOfWaysInWhichArrayCanBeDividedInContinuousSubArrayWithEqualSum(int A, int[] B) {
        int sum = 0;
        for (int i = 0; i < A; i++) {
            sum += B[i];
        }
        ...
        if ((sum % 3) == 0) {
            ...
            // Iterating Over the Array and then Finding the Sum if Prefix and Suffix Array
            for (int i = 0; i < A; i++) {
                tempSum += B[i];
                if (tempSum == requiredSum) {
   ...
   10. C C C       ------> X As Min in C is 1 so Sum will be Greater than 3 so Excluded
    ...
    public static int findingIfThereExistsANobelNumberInArrayWhereNobelNumberIsIfNumberIsEqualToNumberOfItemsGreaterThanThatNumberInArray(int[] A) {
        Arrays.sort(A);
